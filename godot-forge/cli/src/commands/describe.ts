...
const SCHEMAS: Record<string, object> = {
  ...
  "project.validate": {
    ...
    input: null,
    flags: {},
    output: { type: "object", properties: { valid: { type: "boolean" }, issues: { type: "array" }, files_checked: { type: "number" } } },
  },
  "project.clean": {
    ...
    input: null,
    flags: {},
    output: { type: "object", properties: { orphaned_count: { type: "number" }, orphaned: { type: "array" }, deleted: { type: "boolean" } } },
  },
  "project.template.list": {
    ...
    input: null,
    flags: {},
    output: { type: "object", properties: { count: { type: "number" }, templates: { type: "array" } } },
  },
  "script.read": {
    command: "script read <path>",
    description: "Read and parse script structure as JSON",
    input: { type: "argument", properties: { path: { type: "string", required: true } } },
    flags: {},
    output: { type: "object", properties: { path: { type: "string" }, extends: { type: "string" }, class_name: { type: "string" }, signals: { type: "array" }, methods: { type: "array" }, exports: { type: "array" }, line_count: { type: "number" } } },
  },
  "node.duplicate": {
    command: "node duplicate",
    description: "Duplicate a node with its subtree",
    input: { type: "object", properties: { scene: { type: "string", required: true }, path: { type: "string", required: true }, new_name: { type: "string" } } },
    flags: { "--scene": "Scene", "--path": "Source node path", "--new-name": "Name for duplicate" },
    output: { type: "object", properties: { source_path: { type: "string" }, duplicated_name: { type: "string" }, duplicated_node_count: { type: "number" } } },
  },
  "scene.rename": {
    command: "scene rename",
    description: "Rename a scene file and update references",
    input: { type: "object", properties: { scene: { type: "string", required: true }, new_name: { type: "string", required: true } } },
    flags: { "--scene": "Current scene", "--new-name": "New name" },
    output: { type: "object", properties: { old_path: { type: "string" }, new_path: { type: "string" }, references_updated: { type: "number" } } },
  },
  "scene.merge": {
    command: "scene merge",
    description: "Merge nodes from source scenes into target",
    input: { type: "object", properties: { target: { type: "string", required: true }, sources: { type: "array", required: true }, parent: { type: "string", default: "." } } },
    flags: { "--target": "Target scene" },
    output: { type: "object", properties: { target: { type: "string" }, sources_merged: { type: "number" }, nodes_added: { type: "number" } } },
  },
  "resource.delete": {
    command: "resource delete <path>",
    description: "Delete a resource file (requires --force)",
    input: { type: "argument", properties: { path: { type: "string", required: true } } },
    flags: {},
    output: { type: "object", properties: { path: { type: "string" }, deleted: { type: "boolean" }, referenced_by: { type: "number" } } },
  },
  "export.preset.remove": {
    command: "export preset remove",
    description: "Remove an export preset (requires --force)",
    input: { type: "object", properties: { name: { type: "string", required: true } } },
    flags: { "--name": "Preset name" },
    output: { type: "object", properties: { name: { type: "string" }, removed: { type: "boolean" }, remaining_count: { type: "number" } } },
  },
  "node.group.add": {
    command: "node group add",
    description: "Add node to groups",
    input: { type: "object", properties: { scene: { type: "string", required: true }, path: { type: "string", required: true }, groups: { type: "string[]", required: true } } },
    flags: { "--scene": "Scene", "--path": "Node path" },
    output: { type: "object", properties: { node_path: { type: "string" }, groups: { type: "array" } } },
  },
  "node.group.remove": {
    command: "node group remove",
    description: "Remove node from a group",
    input: { type: "object", properties: { scene: { type: "string", required: true }, path: { type: "string", required: true }, group: { type: "string", required: true } } },
    flags: { "--scene": "Scene", "--path": "Node path", "--group": "Group name" },
    output: { type: "object", properties: { node_path: { type: "string" }, removed: { type: "string" } } },
  },
  "node.group.list": {
    command: "node group list",
    description: "List groups for a node",
    input: { type: "object", properties: { scene: { type: "string", required: true }, path: { type: "string", required: true } } },
    flags: { "--scene": "Scene", "--path": "Node path" },
    output: { type: "object", properties: { node_path: { type: "string" }, count: { type: "number" }, groups: { type: "array" } } },
  },
  "project.plugin.list": {
    command: "project plugin list",
    description: "List enabled editor plugins",
    input: null,
    flags: {},
    output: { type: "object", properties: { count: { type: "number" }, plugins: { type: "array" } } },
  },
  "project.plugin.enable": {
    command: "project plugin enable",
    description: "Enable an editor plugin",
    input: { type: "object", properties: { name: { type: "string", required: true } } },
    flags: { "--name": "Plugin path" },
    output: { type: "object", properties: { plugin: { type: "string" }, enabled: { type: "boolean" } } },
  },
  "project.plugin.disable": {
    command: "project plugin disable",
    description: "Disable an editor plugin",
    input: { type: "object", properties: { name: { type: "string", required: true } } },
    flags: { "--name": "Plugin path" },
    output: { type: "object", properties: { plugin: { type: "string" }, disabled: { type: "boolean" } } },
  },
};
