...
3. All scripts via `python3 ${SKILL_DIR}/scripts/<script>.py`

| Script | Purpose | Dependencies |
|--------|---------|-------------|
| `merge-to-pptx.py` | Merge slide images → PPTX (image-only, with speaker notes) | python-pptx, Pillow |
| `merge-to-pdf.py` | Merge slide images → PDF | Pillow |
| `generate-slide-images.py` | Generate slide images from outline (Gemini) | requests |
| `make-editable-pptx.py` | Slide image → editable PPTX (slide/deck/clean 子命令) | openai, python-pptx, Pillow, requests |
| `fix-existing-pptx.py` | Fix text overlap / background artifacts in existing PPTX | python-pptx, Pillow |
...
| `--editable` | After image generation, convert to editable PPTX (OCR + style + inpaint) |
...
test -f .slide-deck/EXTEND.md && echo "project"

# Then user-level
test -f "$HOME/.slide-deck/EXTEND.md" && echo "user"
```

| Path | Location |
|------|----------|
| `.slide-deck/EXTEND.md` | Project directory |
| `$HOME/.slide-deck/EXTEND.md` | User home |
...
options:
  - label: "clean" — Pure solid color, no texture
  - label: "grid" — Subtle grid overlay, technical
  - label: "organic" — Soft textures, hand-drawn feel
  - label: "pixel" — Chunky pixels, 8-bit aesthetic
```
...
options:
  - label: "professional" — Cool-neutral, navy/gold
  - label: "warm" — Earth tones, friendly
  - label: "cool" — Blues, grays, analytical
  - label: "vibrant" — High saturation, bold
```
...
options:
  - label: "geometric" — Modern sans-serif, clean
  - label: "humanist" — Friendly, readable
  - label: "handwritten" — Marker/brush, organic
  - label: "editorial" — Magazine style, dramatic
```
...
options:
  - label: "balanced (Recommended)" — 2-3 key points per slide
  - label: "minimal" — One focus point, maximum whitespace
  - label: "dense" — Multiple data points, compact
```
...
```bash
python3 ${SKILL_DIR}/scripts/merge-to-pptx.py <slide-deck-dir>
python3 ${SKILL_DIR}/scripts/merge-to-pdf.py <slide-deck-dir>
```

### Step 8.5: Editable PPTX (if `--editable`)

Convert slide images to editable PPTX with real text boxes (not just images).

**Pipeline per slide** (banana-slides 工作流):
1. 百度 OCR 提取文字位置
2. 元素裁切 → 独立 PNG（供精确颜色提取）
3. 背景去字（简单填充 + Gemini inpaint）
4. 双路样式提取：crop→精确颜色 / 全图→粗体对齐
5. PPTX 重建：clean_bg 底图 + 透明文本框 + 多色 runs

```bash
# 单张图片
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py slide <image.png>

# 整套 deck
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py deck <slide-deck-dir>

# 指定 slide
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py deck <slide-deck-dir> --slides 6,9,10

# 快速模式（跳过 inpainting 和样式提取）
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py deck <slide-deck-dir> --skip-inpaint --skip-styles

# 跳过裁切，使用简单样式提取（单次 VLM 调用）
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py deck <slide-deck-dir> --skip-crop

# 强制重新处理（忽略缓存）
python3 ${SKILL_DIR}/scripts/make-editable-pptx.py deck <slide-deck-dir> --force
...
**Output**: `{deck-slug}-editable.pptx` — clean_bg 底图 + 透明文本框 + 精确颜色/粗体/对齐 + 多色 runs。

### Step 8.6: Generate Slide Images from Outline

Regenerate specific slide images without going through the full prompt workflow.

```bash
# Specific slides
python3 ${SKILL_DIR}/scripts/generate-slide-images.py <outline-path> --slides 6,9,10
...
Fix text overlap and background artifacts in already-edited PPTX files.

```bash
# Fix and overwrite
python3 ${SKILL_DIR}/scripts/fix-existing-pptx.py <input.pptx>

# Fix and save to new file
python3 ${SKILL_DIR}/scripts/fix-existing-pptx.py <input.pptx> -o <output.pptx>

# Report only (no modification)
python3 ${SKILL_DIR}/scripts/fix-existing-pptx.py <input.pptx> --report-only
```
...
Outline: outline.md
PPTX: {topic-slug}.pptx (image-only)
Editable: {topic-slug}-editable.pptx (if --editable)
PDF: {topic-slug}.pdf
```
